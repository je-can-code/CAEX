...
/*:
 * @plugindesc (v0.2b) The Chrono[trigger] ABS (and ATB) System.
 * @author Moghunter
 *
 * @param Tool Map ID
 * @desc Definition of Tool Map ID.
 * @default 1
 *
 * @param Battle Mode
 ...
 * @desc Sensor range setting to start the battle.
 * @default 3
 *   
 * @param Max Battle Members
 * @desc Definition of the maximum limit of characters in the battle.
 * (1..4)
 * @default 3
 *   
 * @param ATB Mode
 * @desc Definition of ATB type
 * 0 - Wait    1 - Semi Active    2 - Full Active
 * @default 2
 *   
 * @param ATB Max
 * @desc Definition of the maximum value of ATB.
 * @default 1600
 *   
 * @param States Duration
 * @desc Definition of the duration of a shift for the conditions.
 * @default 240
 *   
 * @param Diagonal Movement
 * @desc Enable diagonal motion.
 * @default true
 *  
 * @param Touch Input Mode
 * @desc Enable selection by mouse / touch.
 * @default true
 *   
 * @param Attack Command
 * @desc Toggle the attack command in ABS mode.
 * @default true
 *   
 ...
 *   
 * @param Skill Command
 * @desc Activate the shield command in ABS mode.
 * @default true
 *   
 * @param Item Command
 * @desc Activate the item command in ABS mode.
 * @default true
 *   
 * @param Skill Menu Command
 * @desc Activate the menu command for skills in ABS mode.
 * @default true
 * 
 * @param Item Menu Command
 * @desc Activate the menu command for items in ABS mode.
 * @default true
 * 
 * @param Attack Button
 ...
 * real-time map battle system or complex event interaction puzzles.
 * =============================================================================
 * *** HISTÓRICO ***
 * =============================================================================
 ...
 * *** GRAPHICS ASSETS ***
 * =============================================================================
 * The system images should be saved in the folder:
 *
 * /img/chrono/
 *
 * =============================================================================
 ...
 *
 * Tool Id : X
 *    Sets the event ID of the action. 
 * 	  This action will be activated when using the ability through the menu.
 *
 * Cooperation Skill : X : X : X : ...
 *    Sets the ID of the characters that will be needed to activate the action.
 *
 * Abs Mode
 ...
 * *** SHIELD (NOTETAGS) ***
 * =============================================================================
 * Put this comment to activate the shield action.
 *
 ...
 * 
 * Dead Variable Id : X
 *   - Add +1 to the X variable after the enemy dies.
 *  
 ...
 *   - It leaves the enemy invulnerable to certain actions.
 *    
 * State Icon Y-Axis : Y
 *   - Sets a fixed position of the icon height.
 *
 * =============================================================================
 ...
 *
 * enemy_id : X
 *   - Defines the enemy's Id for the event.
 *
 * walk_nearby : X
 *   - Sets a maximum range of movement area in random walk mode.
 *
 * =============================================================================
 * *** TOOL SYSTEM ***
 * =============================================================================
	Tool System is the event interaction system, these events are called Tool Events 
	which are events that are activated during the game. These Tool Events must be 
	created on a predetermined map separated from the others, by default the Map ID is 1, 
 ...
 * - Defines damage based on Item ID.
 *
 * tool_skill_id : SKILL_ID
 * - Defines damage based on Ability ID.
 *
 * tool_item_cost : ITEM_ID
 * - Activate an item cost for action. (*Archery)
 *
 * tool_duration : X
 ...
 * 
 * tool_area : MODE
 * - Definition of the type of impact area of ​​the action.
 *   - square             (Quadrado)
 ...
 * 
 * tool_range : X
 * - Definition of the size of the collision area of ​​the action.
 * 
 * tool_disable_collision
 * - Disable event collision.
 * 
 * tool_wait_collision : X
 * - Sets a time to activate the action collision.
 * 
 * tool_disable_piercing
 * - Disables crossing target mode.
 *
 * tool_damage_all
 ...
 *       target             - Selecting the target through the cursor.
 *       user               - Position in the user.
 *       move_to_target     - It makes the character go to the target (Chrono mode, not ABS).
 *
 * tool_multihit : LAG_TIME
 * - Activates the multi-hit mode, the value X is the collision time between the hits.
 * 
 * tool_action_times : TIMES : LAG_TIME
 * - Triggers the action X times.
 * 
 * tool_chain_action : ACTION_ID
 * - Activates an action after the character completes the first action.
 * 
 ...
 * 
 * tool_all_directions
 * - Action is activated in all directions.
 * 
 * tool_knockback_duration : X
 * - Time the target is frozen after the hit.
 * 
 * tool_ignore_shield
 * - The collision ignores whether the target is using the shield.
 * 
 * tool_shield_reflect
 ...
 * 
 * tool_diagonal
 * - Allows action to be activated diagonally.
 * 
 * tool_diagonal_angle
 ...
 * - Sets an animation to the user during the invocation phase of the action.
 * 
 * tool_shake
 * - Makes the screen shake by hitting the target.
 * 
 * tool_boomerang : X
 * - Activates the boomerang mode, X is the distance from the action.
 * 
 * tool_hookshot : X
 * - Activates the hookshot mode, X is the distance from the action.
 *
 * tool_user_zoom_effect
 * - Enables zoom animation while using the action. (Requires MOG_Character_Motion plugin)
 *
 ...
 * - Activates the combo system for this action, 
 *   a different action will be activated each time the player presses the required button.
 *
 *           COMMAND TYPE
 ...
 * - Activate the charge system for this action, the action will be activated 
 *   when the player leaves the normal attack button (Weapon Command)
 * 
 * =============================================================================
 ...
 *
 * collision_id : X
 *    - Defines the collision ID of the action event. 
 ...
 *
 * walk_nearby : X
 *    - It makes the event walk in the defined perimeter, 
 * 		this function is useful for creating groups of enemies in Chrono mode.
 * 
 * event sensor : X
 *    - Sets the range of the sensor to activate the action page. 
 * 		This function requires the Event Sensor plugin.
 *
 * battle_sensor : X
 *    - Sets the range of the sensor to activate the battle in Chrono mode.
 * 
 * =============================================================================
 * *** PLUGIN COMMANDS ***
 * =============================================================================
 * 
 * chrono_mode : true
 * - Enable or disable turn-based battle mode (Chrono), 
 * 	 leave "false" if you want to activate ABS mode.
 *
 * atb_mode : 0
 * - Setting the Active Time mode in Chrono mode.
 *   0 - WAIT
 *   1 - SEMI ACTIVE
 *   2 - FULL ACTIVE
 *
 ...
 * - Forces a skill on the character.
 *
 * set_actor_item : ACTOR_ID : TOOL_ID
 * - Forces an item on the character.
 *
 ...
 * - Enables or disables the skills menu.
 *
 * command_item_window : true
 * - Enables or disables the items menu.
 *
 * tool_item_visible : true
 * - Enable or disable item hud.
 *
 * tool_skill_visible : true
 * - Enable or disable skill hud.
 *
 * tool_weapon_visible : true
 * - Enable or disable weapon hud.
 *
 * tool_shield_visible : true
 * - Enable or disable shield hud.
 *
 * tool_collision : true
 * - Enables or disables the collision in the action event.
 *
 * force_damage : Mode
 * - Force the action event to deal damage in Chrono mode.
 *   - target
 *   - area
 *   - all
 *
 ...
 * =============================================================================
 * Commands should be used to create complex action movements. 
 * Use the commands below through the call script command.
 *
 * -----------------------------------------------------------------------------
 * SUBJECT (BASIC)
 * -----------------------------------------------------------------------------
 * These are the basic commands for defining which characters to use.
 *
 * this.user()
 * - Action user definition
 * 
 * this.target()
 ...
 *   use this command when creating movements in the cooperation skills.
 *
 * this.toolEvent()
 * - It is the event of Action.
 *
 * -----------------------------------------------------------------------------
 * ACTION 
 * -----------------------------------------------------------------------------
 ...
 * *** ATB ***
 * =============================================================================
 * The speed of the ATB is based on the agility of the battler and there are 3 ATB modes.
 * 
 ...
 * *** CASTING TIME ***
 * =============================================================================
 * To set the casting time for action, 
 * simply set a value in SPEED greater than zero.
 *  
 */
...
Game_Chrono.prototype.prepareCommandSelectionCN = function(battler,char) {
  ...
  let se = String(Moghunter.ras_commandSE);
